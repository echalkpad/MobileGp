...
//        mHandler.obtainMessage(Constants.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
        ...
        if (isConnected && mState != STATE_CONNECTED) {
            postMessage(EventMessage.STATE_CHANGE, Constants.STATE_CONNECTED);
            isConnected = false;
        }

        if (!isConnecting && mState == STATE_CONNECTING) {
            isConnecting = true;
...
//        Message msg = mHandler.obtainMessage(Constants.MESSAGE_TOAST, MSG_CONNECTION_FAILED, -1);
//        Bundle bundle = new Bundle();
...
//        BluetoothService.this.start();
    ...
    private void connectionLost() {
        // Send a failure message back to the Activity
//        Message msg = mHandler.obtainMessage(Constants.MESSAGE_TOAST, MSG_CONNECTION_LOST, -1);
//        Bundle bundle = new Bundle();
//        bundle.putString(Constants.TOAST, "Device connection was lost");
//        msg.setData(bundle);
//        mHandler.sendMessage(msg);
        postMessage(EventMessage.CONNECTION_LOST, -1);
    }
            ...
            try {
                if (secure) {
                    ...
                    tmp = mAdapter.listenUsingInsecureRfcommWithServiceRecord(
                            NAME_INSECURE, isAndroid ? UUID_ANDROID_DEVICE : UUID_OTHER_DEVICE);
                }
            } catch (IOException e) {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    if (onDataRead != null) {
                        onDataRead.onRead(bytes, buffer);
                    }
//                    Message message = new Message();
//                    message.what = EventMessage.DATA_RECEIVED;
//                    message.arg1 = bytes;
//                    message.obj = buffer;
//                    mEventBus.post(message);
