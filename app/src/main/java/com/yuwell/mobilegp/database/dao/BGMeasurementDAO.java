...
public class BGMeasurementDAO extends BaseDAO<BGMeasurement> {
    ...
    public List<Date> getMeasureDate(Map<String, Object> condition) {
        String personId = (String) condition.get(BGMeasurement.COLUMN_PERSONID);
        Date start = (Date) condition.get(Const.START_DATE);
        Date end = (Date) condition.get(Const.END_DATE);
        String level = (String) condition.get(BGMeasurement.COLUMN_LEVEL);

        StringBuilder sql = new StringBuilder("SELECT measureTime FROM BGMEASUREMENT WHERE ")
                .append(BGMeasurement.COLUMN_DELETE_FLAG).append(" = '").append(BGMeasurement.NORMAL).append("'");
    ...
    public List<BGMeasurement> getList(Map<String, Object> condition) {
        String memberId = (String) condition.get(BGMeasurement.COLUMN_PERSONID);
        Date start = (Date) condition.get(Const.START_DATE);
        Date end = (Date) condition.get(Const.END_DATE);
        String level = (String) condition.get(BGMeasurement.COLUMN_LEVEL);
